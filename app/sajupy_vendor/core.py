...
try:
    ...
    # geopy가 설치되어 있지 않은 환경(오프라인/사설망)을 위한 폴백.
    # 이 경우 KNOWN_CITY_LONGITUDES에 등록된 도시이거나 longitude를
    # 직접 넘겨주는 방식으로만 태양시 보정이 동작한다.
    Nominatim = None
...
# geopy 없이도 바로 쓸 수 있도록 자주 쓰는 도시의 경도를 내장.
# (필요하면 city 대신 longitude를 직접 넘기거나 이 dict를 확장하면 됨)
KNOWN_CITY_LONGITUDES = {
    "seoul": 126.9780, "서울": 126.9780,
    "busan": 129.0756, "부산": 129.0756,
    "incheon": 126.7052, "인천": 126.7052,
    "daegu": 128.6014, "대구": 128.6014,
    "daejeon": 127.3845, "대전": 127.3845,
    "gwangju": 126.8526, "광주": 126.8526,
    "suwon": 127.0286, "수원": 127.0286,
    "jeju": 126.5312, "제주": 126.5312,
    "tokyo": 139.6917, "new york": -74.0060,
    "los angeles": -118.2437, "san francisco": -122.4194,
    "london": -0.1278, "beijing": 116.4074, "shanghai": 121.4737,
}
...
class SajuCalculator:
    ...
    def __init__(self, csv_path: Optional[str] = None):
        ...
        # Geocoder 초기화 (geopy가 없는 환경에서는 내장 도시 목록만 사용)
        self.geolocator = Nominatim(user_agent="saju_calculator") if _GEOPY_AVAILABLE else None
    ...
    def _get_longitude_from_city(self, city: str) -> Optional[float]:
        ...
        Optional[float]
            경도 값 (실패시 None)
        """
        known = KNOWN_CITY_LONGITUDES.get(city.strip().lower())
        if known is not None:
            return known

        if not _GEOPY_AVAILABLE:
            print(f"Warning: geopy가 설치되어 있지 않아 '{city}'의 경도를 조회할 수 없습니다. "
                  f"longitude를 직접 지정하거나 KNOWN_CITY_LONGITUDES에 등록하세요.")
            return None
